...
RUN echo "Building VSecM Init Container on $BUILDPLATFORM targeting $TARGETPLATFORM"
RUN ./hack/docker/buildx.sh ${TARGETARCH} initcontainer

# Use BusyBox as the base image
# FROM busybox:1.36-musl AS spike
FROM busybox:1.36 AS initcontainer
# Redefine the ARG in this stage to make it available
ARG APPVERSION

# Create necessary directories and users
RUN adduser -D -H -u 1000 initcontainer

# Copy the binary from builder
COPY --from=builder /workspace/initcontainer /usr/local/bin/initcontainer

# Change ownership to initcontainer user
RUN chown initcontainer:initcontainer /usr/local/bin/initcontainer

# Ensure the binary is executable
RUN chmod +x /usr/local/bin/initcontainer
...
ENTRYPOINT ["/usr/local/bin/initcontainer"]
