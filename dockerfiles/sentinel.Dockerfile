...
RUN echo "Building VSecM Sentinel on $BUILDPLATFORM targeting $TARGETPLATFORM"
RUN ./hack/docker/buildx.sh ${TARGETARCH} sentinel

# Use BusyBox as the base image
# FROM busybox:1.36-musl AS spike
FROM busybox:1.36 AS sentinel
# Redefine the ARG in this stage to make it available
ARG APPVERSION

# Create necessary directories and users
RUN adduser -D -H -u 1000 sentinel

# Copy the binary from builder
COPY --from=builder /workspace/sentinel /usr/local/bin/sentinel

# Change ownership to initcontainer user
RUN chown sentinel:sentinel /usr/local/bin/sentinel
