...
RUN echo "Building VSecM Safe on $BUILDPLATFORM targeting $TARGETPLATFORM"
RUN ./hack/docker/buildx.sh ${TARGETARCH} safe

# Use BusyBox as the base image
# FROM busybox:1.36-musl AS spike
FROM busybox:1.36 AS safe
# Redefine the ARG in this stage to make it available
ARG APPVERSION

# Create necessary directories and users
RUN adduser -D -H -u 1000 safe

# Copy the binary from builder
COPY --from=builder /workspace/safe /usr/local/bin/safe

# Change ownership to initcontainer user
RUN chown safe:safe /usr/local/bin/safe
